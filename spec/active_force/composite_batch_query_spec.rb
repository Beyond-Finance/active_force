...
describe ActiveForce::CompositeBatchQuery do
  ...
  describe '.call' do
    ...
    it 'defaults to ActiveForce.sfdc_client when no client is provided' do
      default_client = double('default_client')
  ...
  describe '#call' do
    context 'when the response is successful' do
      let(:result_record) { double('result_record') }
      let(:batch_result) do
        double('batch_result', statusCode: 200, result: result_record)
      end
    ...
    context 'when the response is an error' do
      ...
      let(:batch_result) do
        double('batch_result', statusCode: 400, result: error_body)
      end
