...
describe ActiveForce::ActiveQuery do
  ...
  describe 'condition mapping' do
    ...
    it 'transforms an array to a WHERE/IN clause' do
      active_query.where(field: %w[foo bar])
    ...
    describe 'bind parameters' do
      ...
      it 'complains when there given an incorrect number of bind parameters' do
        ...
        end.to raise_error(ActiveForce::PreparedStatementInvalid, 'wrong number of bind variables (2 for 3)')
      end
      ...
      context 'named bind parameters' do
        ...
        it 'accepts multiple bind parameters' do
          active_query.where('Field__c = :field AND Other_Field__c = :other_field AND Name = :name', field: 123,
                                                                                                     other_field: 321, name: 'Bob')
          expect(active_query.to_s).to eq("SELECT Id FROM table_name WHERE (Field__c = 123 AND Other_Field__c = 321 AND Name = 'Bob')")
        end

        it 'accepts multiple bind parameters orderless' do
          active_query.where('Field__c = :field AND Other_Field__c = :other_field AND Name = :name', name: 'Bob',
                                                                                                     other_field: 321, field: 123)
          expect(active_query.to_s).to eq("SELECT Id FROM table_name WHERE (Field__c = 123 AND Other_Field__c = 321 AND Name = 'Bob')")
        end
        ...
        it 'complains when there given an incorrect number of bind parameters' do
          expect do
            active_query.where('Field__c = :field AND Other_Field__c = :other_field AND Name = :name', field: 123,
                                                                                                       other_field: 321)
          end.to raise_error(ActiveForce::PreparedStatementInvalid,
                             'missing value for :name in Field__c = :field AND Other_Field__c = :other_field AND Name = :name')
        end
      end
    end
  end
  ...
  describe 'prevent SOQL injection attacks' do
    ...
    it 'escapes quotes and backslashes in bind parameters' do
      active_query.where(
        'Backslash_Field__c = :backslash_field AND NumberField = :number_field AND QuoteField = :quote_field', number_field: number_input, backslash_field: backslash_input, quote_field: quote_input
      )
      expect(active_query.to_s).to eq(expected_query)
    end

    it 'escapes quotes and backslashes in named bind parameters' do
      active_query.where('Backslash_Field__c = ? AND NumberField = ? AND QuoteField = ?', backslash_input,
                         number_input, quote_input)
      expect(active_query.to_s).to eq(expected_query)
    end
